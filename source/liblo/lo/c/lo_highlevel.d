...
public import lo.c.lo_osc_types;
public import lo.c.lo_definitions;
import lo.c.lo_lowlevel: lo_send_internal, lo_send_timestamped_internal, lo_send_from_internal;
 ...
 * \return -1 on failure.
 */
int lo_send(string file = __FILE__, int line = __LINE__, A...)
    (lo_address targ, const(char) *path, const(char) *type, A a)
{
  return lo_send_internal(targ, file.toStringz, line, path, type, a, LO_MARKER_A, LO_MARKER_B);
}
 ...
 */
int lo_send_from(string file = __FILE__, int line = __LINE__, A...)
    (lo_address targ, lo_server from, lo_timetag ts, const(char) *path, const(char) *type, A a)
{
  return lo_send_from_internal(targ, from, file.toStringz, line, ts, path, type, a, LO_MARKER_A, LO_MARKER_B);
}
 ...
 * \return on success, the number of bytes sent, or -1 on failure.
 */
int lo_send_timestamped(string file = __FILE__, int line = __LINE__, A...)
    (lo_address targ, lo_timetag ts, const(char) *path, const(char) *type, A a)
{
  return lo_send_timestamped_internal(targ, file.toStringz, line, ts, path, type, a, LO_MARKER_A, LO_MARKER_B);
}
